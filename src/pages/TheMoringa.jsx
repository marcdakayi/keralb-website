...
const product = {
  ...
  description: 'Le moringa, surnommé "l\'arbre de vie" en Afrique, dans une tisane bio premium. 20 sachets pour un rituel bien-être quotidien, produit au Cameroun.',
  volume: '20 sachets',
  ingredients: ['Feuilles de Moringa'],
  bienfaits: [
    { icon: '🌿', title: 'Nutrition', desc: '92 nutriments essentiels' },
    { icon: '⚡', title: 'Énergie', desc: 'Combat la fatigue chronique' },
    { icon: '🛡️', title: 'Immunité', desc: 'Antioxydants puissants' },
    { icon: '💚', title: 'Détox', desc: "Purifie l'organisme naturellement" },
  ],
  price: '3000 FCFA',
  color: '#3A7A3A',
  imageFront: '/images/produits/moringa-avant.png',
  imageBack: '/images/produits/moringa-arriere1.png',
}
    ...
    >
      ...
      >
        <div
          ...
          style={{ background: `radial-gradient(ellipse 60% 60% at 70% 50%, rgba(58,122,58,0.12) 0%, transparent 70%)` }}
        />
        ...
        <div className="flex flex-col justify-center px-8 md:px-16 lg:px-24 pt-32 pb-16 relative z-10">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:gap-4 w-full sm:w-auto">
            <a
              href="https://wa.me/237656783732?text=Je%20commande%20le%20Th%C3%A9%20Moringa%20Keral-B"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-3 px-8 py-4 font-body font-medium text-xs tracking-[0.2em] uppercase text-white hover:opacity-90 transition-all duration-500 hover:-translate-y-1"
        ...
        <div className="flex items-center justify-center relative pt-16 md:pt-0 pb-16 md:pb-0">
          <div ref={bottleRef} className="relative w-36 md:w-64 lg:w-72" style={{ transformStyle: 'preserve-3d' }}>
            <img
              ...
              style={{ filter: 'drop-shadow(0 40px 80px rgba(58,122,58,0.4))' }}
            />
          </div>
          ...
          <div className="absolute w-48 h-48 md:w-72 md:h-72 rounded-full" style={{ border: `1px solid rgba(58,122,58,0.1)` }} />
        </div>
      </section>
      ...
      <section className="py-24 px-6 text-center border-t border-white/5" style={{ background: '#080808' }}>
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-3 px-12 py-5 font-body font-medium text-xs tracking-[0.25em] uppercase text-white hover:opacity-90 transition-all duration-300 hover:-translate-y-1"
          style={{ background: product.color }}
        >
