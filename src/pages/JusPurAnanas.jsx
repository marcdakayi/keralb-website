...
const product = {
  ...
  description: "Un jus d'ananas 100% naturel, sans sucre ajouté, sans conservateur. Juste l'ananas camerounais dans toute sa pureté.",
  volume: '300ml',
  ingredients: ['Ananas'],
  bienfaits: [
    { icon: '💧', title: 'Hydratation', desc: 'Riche en eau naturelle' },
    { icon: '⚡', title: 'Énergie', desc: 'Sucres naturels du fruit' },
    { icon: '🛡️', title: 'Immunité', desc: 'Vitamine C naturelle' },
    { icon: '✨', title: 'Digestion', desc: 'Bromélaïne naturelle' },
  ],
  price: '2500 FCFA',
  color: '#E8C547',
  imageFront: '/images/produits/ananas-avant.png',
  imageBack: '/images/produits/ananas-arriere.png',
}
...
export default function JusPurAnanas() {
  ...
  return (
    ...
    >
          ...
          >
            {product.name.split('\n').map((line, i) => (
              <span key={i} className="block">
                {i === 1
                  ? <span style={{ color: product.color }}>{line}</span>
                  : line
                }
              </span>
            ))}
          </h1>
          <p className="font-accent italic text-white/50 text-xl mb-8">
            {product.tagline}
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <a
              ...
              className="inline-flex items-center justify-center gap-3
                         px-8 py-4 font-body font-medium text-xs
                         tracking-[0.2em] uppercase
                         text-[#050505] hover:opacity-90
            ...
            <a
              href="/"
              className="inline-flex items-center justify-center gap-3
                         px-8 py-4 border border-white/15
                         text-white/50 font-body text-xs
                         tracking-[0.2em] uppercase
        ...
        <div className="flex items-center justify-center relative
          ...
          >
            <img
              ...
              style={{
                filter: 'drop-shadow(0 40px 80px rgba(232,197,71,0.3))',
              }}
            />
          </div>
          ...
          <div
            className="absolute w-64 h-64 md:w-96 md:h-96 rounded-full
                       border border-white/5 animate-spin"
            style={{ animationDuration: '20s' }}
          />
          <div
            className="absolute w-48 h-48 md:w-72 md:h-72 rounded-full"
            style={{ border: `1px solid rgba(232,197,71,0.08)` }}
          />
        </div>
      </section>
      ...
      <section
        ref={infoRef}
        className="py-32 px-6 md:px-16 lg:px-32 border-t border-white/5"
      >
          ...
          <div className="reveal-item">
            ...
            <div className="grid grid-cols-2 gap-4">
              {product.bienfaits.map((b, i) => (
                <div
                  key={i}
                  className="p-6 border border-white/5
      ...
      >
        <p className="font-display text-white/20 tracking-[0.5em]
        ...
        <a
          ...
          className="inline-flex items-center gap-3 px-12 py-5
                     font-body font-medium text-xs tracking-[0.25em] uppercase
                     text-[#050505] hover:opacity-90
